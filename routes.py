...
def admin_dashboard():
    """Modern admin dashboard with clean interface and real data"""
    if not session.get('admin_logged_in'):
        flash('Please log in to access the admin dashboard.', 'error')
        return redirect(url_for('admin_login'))

    try:
        # Get today's stats from actual data
        ...
        # Calculate week revenue from accepted quotes
        ...
        new_contacts = len([c for c in contacts if 
                           datetime.fromisoformat(c.get('created_date', '2025-01-01') + 'T00:00:00').date() >= yesterday.date()])
        ...
        return render_template('admin/dashboard/main.html', **dashboard_data)
    
    except Exception as e:
        logging.error(f"Modern dashboard error: {e}")
        # Return minimal dashboard with safe defaults
        return render_template('admin/dashboard/main.html',
...
# Twilio configuration removed - using MailerLite for notifications
...
# Duplicate routes removed - using existing implementations
